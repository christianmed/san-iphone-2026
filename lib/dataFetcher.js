 ...
 */
/**
 * Extrae y normaliza el nombre del color asignado para el iPhone 18 Pro Max
 */
function extractColorName(colorStr, imageStr) {
  const combined = ((colorStr || '') + ' ' + (imageStr || '')).toLowerCase();
  if (combined.includes('glaciar')) return 'Glaciar';
  if (combined.includes('negro')) return 'Negro';
  if (combined.includes('borgo') || combined.includes('borgona')) return 'Borgoña';
  if (combined.includes('plata')) return 'Plata';
...
function formatTandaData({ config = [], dbUsuarios = [], registroPagos = [], dashboard = [], kpis = [], logErrores = [] }) {
  ...
  const globalKpis = {
    ...
    proximaMoto: (() => {
      const pNombre = String(kpiRow['Próximo Participante'] || '');
      const pUser = Object.values(userMap).find(u => (u.Nombre && u.Nombre.toLowerCase() === pNombre.toLowerCase()) || (u.ID && u.ID.toLowerCase() === pNombre.toLowerCase()));
      const pColor = pUser ? extractColorName(pUser['Color / Capacidad'] || pUser['Color'], pUser.Imagen) : '';
      const base = String(kpiRow['Próximo iPhone'] || kpiRow['Próxima Moto'] || 'iPhone 18 Pro Max');
      return pColor && !base.includes('-') ? `${base} - ${pColor}` : base;
    })(),
    costoProximaMoto: cleanNumber(kpiRow['Costo Próximo iPhone'] || kpiRow['Costo Próxima Moto']),
    fondoProximaMoto: cleanNumber(kpiRow['Fondo Próximo iPhone'] || kpiRow['Fondo Próxima Moto']),
  ...
  const participants = dashboard.map((item, idx) => {
    ...
    const costoMoto = cleanNumber(item['Costo iPhone'] || userDetail['Costo iPhone'] || item['Costo Moto'] || userDetail['Costo Moto']);
    ...
    const rawPin = String(userDetail.PIN || userDetail.pin || userDetail['PIN'] || '').trim();
    const defaultPin = String(id.replace(/\D/g, '') || idx + 1).padStart(8, '0');
    const pin = rawPin ? rawPin : defaultPin;

    const baseModelo = userDetail['Modelo iPhone'] || userDetail['Modelo Moto'] || 'iPhone 18 Pro Max';
    const color = extractColorName(userDetail['Color / Capacidad'] || userDetail['Color'], userDetail.Imagen);
    const modeloCompleto = `${baseModelo} - ${color}`;
    ...
    return {
      ...
      modeloMoto: modeloCompleto,
      modeloBase: baseModelo,
      modeloCompleto,
      color,
      colorCapacidad: userDetail['Color / Capacidad'] || userDetail['Color'] || '',
      imagenMoto: userDetail.Imagen || '/img/plata.jpg',
      ...
      cuotasVencidas,
      porcentajeProgreso,
      estatusMoto: item['Estatus Entrega'] || item['Estatus Moto'] || userDetail['Estatus Entrega'] || userDetail['Estatus Moto'] || 'Pendiente',
